$(function () {
    ...
    var YourFamily = $("#YourFamily").DataTable({
        ...
        "drawCallback": function (settings) {
            ...
            localStorage.FamilyMembers = JSON.stringify(FamilyMembers);
        },
        "data": (localStorage.FamilyMembers) ? JSON.parse(localStorage.FamilyMembers) : null
    });
    $('#YourFamily tbody').on("click", ".btn-danger", function () {
        YourFamily.row($(this).parents('tr')).remove().draw();
    });
    $('#YourFamily tbody').on("click", ".btn-primary", function () {
        var rowData = YourFamily.row($(this).parents('tr')).data();
    ...
    var familyMemberForm = $("#familyMemberForm").validate({
        "submitHandler": function (form) {
            if ($("#familyModalPrimary").val() === "Update Family Member") {
                YourFamily.row.add($("#familyModal").data("original")).draw();
                ...
                YourFamily.row.add(familyMember).draw();
            }
            $("#familyModal").modal("hide");
        }
    });
    $("#familyModal").on("hidden.bs.modal", function () {
        familyMemberForm.resetForm();
        document.getElementById("familyMemberForm").reset();
        $("#familyModal").data("original", "");
    });
    $("#familyForm").validate({
        "submitHandler": function (form) {
            var Family = (localStorage.FamilyMembers) ? JSON.parse(localStorage.FamilyMembers) : [];
            var Member = {
                "title": $("#yourTitle").val(),
            ...
            var encodedData = window.btoa(JSON.stringify(Family));
            var href = window.location.href.split("/");
            href.pop();
            if ($("#familyForm").context.activeElement.value === "Add Details - TABLES") {
                href.push("old.html");
            } else {
                href.push("new.html");
            }
